...
type argErrorStu struct {
	arg  int
	prob string
}

func (e *argErrorStu) Error() string {
	return fmt.Sprintf("%d - %s", e.arg, e.prob)
}
func f2(arg int) (int, error) {

	var err error = errors.New("test another error")
	var ar2 argErrorStu = argErrorStu{arg, "can't work with it 44"}

	if arg == 42 {
		//在这个例子中，我们使用 &argError 语法来建立一个新的结构体，并提供了 arg 和 prob 这个两个字段的值。
		return -1, &argErrorStu{arg, "can't work with it 42"}
	} else if arg == 43 {
		return -1, err
	} else if arg == 44 {
		return -1, &ar2
...
/*
* 错误处理：Go 语言使用一个独立的、明确的返回值来传递错误信息的。
